...
import { SubmitReportInput, SubmittedReport } from './interfaces/report.interfaces';
...
export class ReportsService {
    ...
    async downloadFileData(userId: number, reportId: number) {
        const reportFile = await this.reportsRepo.findReportWithFile(reportId, userId);

        if (!reportFile?.fileData?.data)
            throw new FileNotFoundException("File not found in database", { reportId });
