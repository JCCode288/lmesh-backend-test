...
export class ReportsController {
    ...
    /**
     * @description endpoint to delete report based on it's id. deleted report will be cascading to it's respective analysis and file data
     * @param user authenticated user 
     * @param reportId report id to delete
     */
    @Delete('/:id')
    @ResponseMessage('Report deleted')
    async deleteReport(@CurrentUser() user: AuthUser, @Param('id') reportId: number) {
        if (!reportId) throw new InvalidPayload('Invalid ID');

        return this.reportSvc.deleteReport(user.userId, reportId);
    }
