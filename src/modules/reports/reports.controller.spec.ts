...
describe('ReportsController', () => {
    ...
    const buildReq = (file: any) => ({ file: jest.fn().mockResolvedValue(file) }) as any;
    ...
    describe('analyzeReport', () => {
        it('submits a valid upload', async () => {
            reportSvc.submit.mockResolvedValue({ reportId: 1, analysisId: 10, status: 'PENDING' });

            const result = await controller.analyzeReport(buildReq(buildFile()), user as any);

            expect(result).toEqual({ reportId: 1, analysisId: 10, status: 'PENDING' });
            expect(reportSvc.submit).toHaveBeenCalledWith(
                expect.objectContaining({ userId: 7, contentType: 'json', partNumber: 'P1', plantCode: 'PL1' }),
            );
        });
    ...
    describe('resolveType (private)', () => {
        ...
        it('throws on an unsupported type', () => {
            expect(() => resolve('data.txt', 'text/plain')).toThrow(FileUploadException);
        });
    });

    describe('resolveStatus (private)', () => {
        const resolve = (status: string) => (controller as any).resolveStatus(status);
