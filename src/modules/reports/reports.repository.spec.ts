...
describe('ReportsRepository', () => {
  ...
  let prisma: {
    reports: { create: jest.Mock; findFirst: jest.Mock; findMany: jest.Mock };
  };
    ...
    expect(prisma.reports.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          ...
          createdBy: 3,
          fileData: {
            create: expect.objectContaining({ data: expect.any(Uint8Array) }),
          },
          analysis: { create: {} },
        }),
  ...
  it('listReports filters by analysis status when provided', async () => {
    ...
    expect(prisma.reports.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { createdBy: 9, analysis: { status: 'FAILED' } },
      }),
    );
  });
});
