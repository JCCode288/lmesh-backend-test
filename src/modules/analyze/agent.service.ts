...
export class AgentService {
  ...
  constructor(private readonly configSvc: ConfigService) {
    ...
    const modelName =
      this.configSvc.get<string>('GEMINI_MODEL') || 'gemini-2.5-flash';
    const temperature = parseFloat(
      this.configSvc.get<string>('GEMINI_TEMPERATURE') || '0',
    );
  ...
  async analyze(data: string): Promise<AnalysisResult> {
    ...
    const structuredModel =
      this.model.withStructuredOutput(analysisResultSchema);
