...
describe('AuthService', () => {
    ...
    describe('validateUser', () => {
        it('returns the user without the password hash on correct credentials', async () => {
            authRepo.findByUsername.mockResolvedValue({ id: 1, username: 'alice', password: passwordHash });
        ...
        it('throws INVALID_CREDENTIALS on a wrong password', async () => {
            ...
            await expect(service.validateUser('alice', 'wrong')).rejects.toMatchObject({
        ...
        it('throws INVALID_CREDENTIALS for an unknown user', async () => {
            ...
            await expect(service.validateUser('ghost', 'password123')).rejects.toBeInstanceOf(
    ...
    describe('login', () => {
        it('issues a signed access token with sub and username claims', async () => {
            authRepo.findByUsername.mockResolvedValue({ id: 7, username: 'bob', password: passwordHash });
            jwtService.signAsync.mockResolvedValue('signed.jwt.token');

            const result = await service.login({ username: 'bob', password: 'password123' });

            expect(result).toEqual({ access_token: 'signed.jwt.token' });
            expect(jwtService.signAsync).toHaveBeenCalledWith({ sub: 7, username: 'bob' });
        });
    });
});
