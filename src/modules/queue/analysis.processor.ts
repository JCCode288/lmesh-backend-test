...
export class AnalysisProcessor extends WorkerHost {
  ...
  async process(job: Job<AnalysisJob>) {
    ...
    this.logger.log(
      `Analyzing #${analysisId} (attempt ${job.attemptsMade + 1})`,
    );
    ...
    if (!fileData)
      throw new UnrecoverableError(
        `No file data found for analysis #${analysisId}`,
      );
  ...
  async onCompleted(job: Job<AnalysisJob>, result: AnalysisResult) {
    ...
    if (!result.success) {
      await this.analyzeRepo.markFailed(
        analysisId,
  ...
  async onFailed(job: Job<AnalysisJob>, err: Error) {
    ...
    if (job.attemptsMade >= maxAttempts) {
      this.logger.error(
        `Analysis #${analysisId} failed permanently: ${err.message}`,
      );
    ...
    this.logger.error(
      `Analysis #${analysisId} failing: ${err.message}. Retrying`,
    );
  }
}
